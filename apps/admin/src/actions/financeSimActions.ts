...
import { eq, and, count, avg, sql, inArray, isNotNull } from "drizzle-orm";
...
export const getFinanceSimClasses = withAuth(async (_user) => {
  ...
  // 수업별 학생 수 & 완료 학생 수를 각각 단일 쿼리로 집계 (2N+1 → 3 queries)
  const [studentCounts, completedCounts] = await Promise.all([
    db
      .select({ classId: guests.classId, count: count() })
      .from(guests)
      .where(inArray(guests.classId, classIds))
      .groupBy(guests.classId),
    db
      .select({ classId: financeSimulations.classId, count: count() })
      .from(financeSimulations)
      .where(
        and(
          inArray(financeSimulations.classId, classIds),
          isNotNull(financeSimulations.completedAt),
        ),
      )
      .groupBy(financeSimulations.classId),
  ]);

  const studentCountMap = new Map(
    studentCounts.map((r) => [r.classId, r.count]),
  );
  const completedCountMap = new Map(
    completedCounts.map((r) => [r.classId, r.count]),
  );

  return result.map((cls) => ({
    ...cls,
    studentCount: studentCountMap.get(cls.id) ?? 0,
    completedCount: completedCountMap.get(cls.id) ?? 0,
  }));
});

// 수업별 결과 요약 집계
export const getClassSimulationSummary = withAuth(
  async (_user, classId: string) => {
    // 기본 수업 정보
...
// 개별 학생 결과 목록
export const getClassSimulationDetails = withAuth(
  async (_user, classId: string) => {
...
// 학생 개별 시뮬레이션 상세 조회
export const getStudentSimulation = withAuth(
  async (_user, simulationId: string) => {
